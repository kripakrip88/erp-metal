...
### erp-metal
- [feat] nesting.html — модуль раскроя материала (Nesting v2)
  - Листовой раскрой: 2D shelf packing с опциональным поворотом деталей
  - Линейный раскрой: 1D FFD bin packing для профилей и труб
  - Дедупликация раскроев: одинаковые карты объединяются с множителем ×N
  - SVG-визуализация каждого листа/прутка с легендой деталей
  - Итоговая таблица: вес, стоимость, % использования по всем материалам
  - Поиск по каталогу /api/materials с JWT-авторизацией, fallback на демо-данные
  - Печать карты раскроя через window.print()
- [feat] nav.js — добавлен пункт «Раскрой» в группу Workflow
